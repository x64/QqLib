...
##### 4. qq_lock 
Макрос для использования критический секций на основе мьютекса. Работает глобально для всех потоков(!). Т.е. использовать можно только для работы со статическими переменными или объктами типа Singleton. Пример использования. Допустим, необходимо обеспечить эксклюзивное исполнение какого-либо кода. Для этого можно использовать ```qq_lock```:
```C++
