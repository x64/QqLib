...
```friend Class operator - (Class const & lh, Class const & rh)``` – the ```Class``` instance containing the difference ```lh.m_index – rh.m_index```. If ```lh.m_index – rh.m_index``` is less than zero or greater than ```Class::_lastIndex()```, the transitive method will throw an ```std::out_of_range``` exception.  
